...
COLPQTorus::COLPQTorus(CPoint3D _center, float _outRadius, float _inRadius, int _secDetails, int _skelDetails,float mPParameter = 3,float mQParameter = 8)
{
  mCenter = _center;
  mOuterRadius = _outRadius;
  mInnerRadius = _inRadius;
...
CObject3D* COLPQTorus::Generate()
{
  ...
  for(int i = 0; i < mSectionDetails; i++)
  {
    lCenterRadius = lCenterRadius  -1 + 0.5 * (2 + sin (mQParameter * i * lSectionAngle * M_PI / 180));
    lSection[i].SetOriginal
    (
      CPoint3D
      (
//        lCenterRadius + mCenter.GetX() + lSectionRadius * cos(i * lSectionAngle * M_PI / 180.0),
        mCenter.GetX() + lSectionRadius * cos(mPParameter * i * lSectionAngle * M_PI / 180.0),
	mCenter.GetY() + lSectionRadius * cos(mQParameter * i * lSectionAngle * M_PI / 180.0),
	mCenter.GetZ() + lSectionRadius * sin(mPParameter * i * lSectionAngle * M_PI / 180.0)
      )
    );
  }
...
COLTrunchiCone::COLTrunchiCone(CPoint3D _center, float _height, float _smallradius, float _bigradius, int _details)
{
  mCenter = _center;
  mHeight = _height;
  mSmallRadius = _smallradius;
  mBigRadius = _bigradius;
  mDetails = _details;
}
//-----------------------------------------------------------------

CObject3D* COLTrunchiCone::Generate()
{
  CObject3D* lObj = new CObject3D();
  CVertex lPrevV1, lV1, lV2, lPrevV2, lF1, lF2, lTopCenter, lCenter;
  float lStep = 360.0 / mDetails;
  ...
  lCenter.SetOriginal(mCenter);
  lTopCenter.SetOriginal(CPoint3D(mCenter.GetX(), mCenter.GetY(), mCenter.GetZ() + mHeight));
  lPrevV1.SetOriginal(CPoint3D(mCenter.GetX() + mSmallRadius, mCenter.GetY(), mCenter.GetZ()));
  lPrevV2.SetOriginal(CPoint3D(mCenter.GetX() + mBigRadius, mCenter.GetY(), mCenter.GetZ() + mHeight));
  ...
  for(float lAngle = lStep; lAngle < 360.0; lAngle += lStep)
  {
    lV1.SetOriginal(
       CPoint3D(
       mCenter.GetX() + mSmallRadius * cos(lAngle * M_PI / 180),
       mCenter.GetY() + mSmallRadius * sin(lAngle * M_PI / 180),
       mCenter.GetZ()
       )
    );
    ...
    lV2.SetOriginal(
       CPoint3D(
       mCenter.GetX() + mBigRadius * cos(lAngle * M_PI / 180),
       mCenter.GetY() + mBigRadius * sin(lAngle * M_PI / 180),
       mCenter.GetZ() + mHeight
       )
    );
...
COLPiramid::COLPiramid(CPoint3D _center, float _height, float laturabazei, int _details)
{
  mCenter = _center;
  mHeight = _height;
  mBaseSideSize = laturabazei;
  mDetails = _details;
}
//-----------------------------------------------------------------

CObject3D* COLPiramid::Generate()
{
  CVertex lPoints[4];
  CVertex lTop;
  float lHS = mBaseSideSize / 2;

  // init the points
  lPoints[0].SetOriginal(CPoint3D(mCenter.GetX() - lHS, mCenter.GetY() - lHS, mCenter.GetZ() ));
  lPoints[1].SetOriginal(CPoint3D(mCenter.GetX() - lHS, mCenter.GetY() + lHS, mCenter.GetZ() ));
  lPoints[2].SetOriginal(CPoint3D(mCenter.GetX() + lHS, mCenter.GetY() + lHS, mCenter.GetZ() ));
  lPoints[3].SetOriginal(CPoint3D(mCenter.GetX() + lHS, mCenter.GetY() - lHS, mCenter.GetZ() ));
  lTop.SetOriginal(CPoint3D(mCenter.GetX() , mCenter.GetY() , mCenter.GetZ() + mHeight ));
  ...
  // add faces
  lObj->AddSurface(lTop, lPoints[0], lPoints[1]);
  lObj->AddSurface(lTop, lPoints[1], lPoints[2]);
  lObj->AddSurface(lTop, lPoints[2], lPoints[3]);
  lObj->AddSurface(lTop, lPoints[3], lPoints[0]);

  // add bottom
  lObj->AddSurface(lPoint[0], lPoints[3], lPoints[2]);
  lObj->AddSurface(lPoint[0], lPoints[1], lPoints[2]);
